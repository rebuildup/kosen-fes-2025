...
export const UnifiedCard = React.memo(({
  ...
  const getCardClasses = () => {
    ...
    switch (variant) {
      ...
      case "timeline":
        return `schedule-card group cursor-pointer relative overflow-hidden transition-all duration-300 ease-out border rounded-lg ${isHovered ? "h-64" : "h-32"}`;
      case "compact":
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/20 to-transparent text-white">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-3">
            ...
            <h3 className="schedule-card-title mb-1">{formatText(item.title)}</h3>
          ...
          >
            <div className="space-y-3 max-h-full overflow-y-auto">
              <h3 className="text-lg font-semibold">{formatText(item.title)}</h3>
    ...
    const cardContent = (
      ...
      >
        ...
        <div className="card-image-container w-32 h-32 flex-shrink-0">
          ...
          <button
            onClick={handleBookmarkClick}
            className={`card-bookmark-button ${isBookmarked(item.id) ? "bookmarked" : ""}`}
            aria-label={
        ...
        <div className="card-content flex-1">
          ...
          {organization && (
            <div className="card-meta">
              <PeopleIcon size={16} />
              <span>{organizationLabel}: {formatText(organization)}</span>
            </div>
          )}
          ...
          {showTags && item.tags && item.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              ...
              {item.tags.length > 3 && (
                <span className="text-xs opacity-60">+{item.tags.length - 3}</span>
              )}
            </div>
          )}
        </div>
