...
const BookmarkList: React.FC<BookmarkListProps> = ({
  ...
  const filteredBookmarks = activeGroup === 'all'
    ? bookmarks
    : bookmarks.filter(bookmark => bookmark.type === activeGroup);
  
  // Count items by type
  const eventCount = bookmarks.filter(item => item.type === 'event').length;
  const exhibitionCount = bookmarks.filter(item => item.type === 'exhibition').length;
  const foodStallCount = bookmarks.filter(item => item.type === 'foodStall').length;
  ...
  if (bookmarks.length === 0) {
    return (
      <div className={`bookmark-list-empty ${className} text-center py-12`}>
        ...
        <h3 className="text-lg font-medium mb-2">{t('bookmarks.noBookmarks')}</h3>
          ...
          {filteredBookmarks.map(bookmark => (
            ...
            >
              <div className="flex items-center p-3 bg-background-card rounded-lg hover:bg-background-card-hover transition-colors duration-200">
                <div className="w-12 h-12 rounded-md ove